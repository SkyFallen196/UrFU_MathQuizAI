...
def handle_vector_length_mode():
    x = random.randint(0, 20)
    y = random.randint(0, 20)
    z = random.randint(0, 20)

    template = f"""Составь задание без вариантов ответа и без решения, в котором требуется вычислить длину вектора.

Требования:

- Используй следующий шаблон:
Вычислите длину вектора A = ({x}, {y}, {z}).
- Не добавляй кавычки в начале и конце предложения.
- Все числа вектора – целые числа от 0 до 20.
- Избегай дополнительных комментариев или отклонений от шаблона.

Пример:

Вычислите длину вектора A = (3, 4, 12).
"""
    handle_template_mode(template, f"Вычисление длины вектора", operation="vector_length")


def handle_function_evaluation_mode():
    template = """Составь задание без вариантов ответа и без решения, в котором требуется вычислить значение функции в заданной точке.

Требования:

- Используй следующий шаблон:
"Дана функция f(x) = {function}. Найдите значение функции в точке x = {point}."
- Функция должна быть простой, например, линейной, квадратичной или тригонометрической (sin, cos, tg, ctg).
- Точка x должна быть целым числом от -10 до 10.
- Функция и точка должны быть такими, чтобы значение функции в этой точке легко вычислялось вручную.
- Не добавляй кавычки в начале и конце предложения.
- Избегай дополнительных комментариев или отклонений от шаблона.

Примеры:

1. Дана функция f(x) = x^2 + 3x - 5. Найдите значение функции в точке x = 2.
2. Дана функция f(x) = sin(x). Найдите значение функции в точке x = π/2.
3. Дана функция f(x) = 2x + 4. Найдите значение функции в точке x = -3.
"""
    handle_template_mode(template, "Вычисление значения функции в точке", operation="function_evaluation")


def handle_module_of_complex_number():
    template = """Составь задание без вариантов ответа и без решения, в котором требуется вычислить модуль комплексного числа.

Требования:

- Используй следующий шаблон:
"Дано комплексное число z = {complex_number}. Найдите его модуль."
- Комплексное число должно быть в виде a + bi, где a и b – целые числа от -10 до 10.
- Результат модуля должен быть целым числом.
- Не добавляй кавычки в начале и конце предложения.
- Избегай дополнительных комментариев или отклонений от шаблона.

Примеры:

1. Дано комплексное число z = 3 + 4i. Найдите его модуль.
2. Дано комплексное число z = -6 + 8i. Найдите его модуль.
3. Дано комплексное число z = 5 - 12i. Найдите его модуль.
"""
    handle_template_mode(template, "Вычисление модуля комплексного числа", operation="complex_module")


def handle_derivative_evaluation():
    template = """Составь задание без вариантов ответа и без решения, в котором требуется вычислить производную функции в заданной точке.

Требования:

- Используй следующий шаблон:
"Дана функция f(x) = {function}. Найдите значение её производной в точке x = {point}."
- Функция должна быть линейной, квадратичной, кубической или простой степенной (например, x^n).
- Точка x должна быть целым числом от -10 до 10.
- Выражение функции и её производной в указанной точке должны быть легко вычисляемыми вручную.
- Не добавляй кавычки в начале и конце предложения.
- Избегай дополнительных комментариев или отклонений от шаблона.

Примеры:

1. Дана функция f(x) = 2x + 3. Найдите значение её производной в точке x = 5.
2. Дана функция f(x) = x^2 - 4x + 1. Найдите значение её производной в точке x = -2.
3. Дана функция f(x) = 3x^3. Найдите значение её производной в точке x = 1.
"""
    handle_template_mode(template, "Вычисление производной функции в точке", operation="derivative_evaluation")
